#Função para calcular o digito verificador do CPF parcial
def calcular_digito_verificador(cpf_parcial):
   ...
   if resto_divisao < 2:
       return '0' # Se o resto da divisão for menor que 2, retorna '9'
   else:
...
def verificar_cpf(cpf):
   ...
   #Verifica se o CPF tem 11 digitos e se são todos números
   ...
   return cpf == cpf_calculado # Retorna True se o CPF original for igual ao CPF calculado com os digitos verificadores
...
def main():
   ...
   if verificar_cpf(cpf): #Verifica se o CPF é válido
       print("CPF válido!") # Se for válido, exibe essa mensagen
   else:
       print("CPF inválidol") # Se não for válido, exibe essa mensagen
